...
class LdViewSerializer(ABC):
    ...
    def serialize(self, view: View):
        # serialize base
        with self._templater('ldviews/metadata.ttl.jinja', f"view.{view.id}.ttl") as engine:
            engine.template({'view': view})

        with self._templater('ldviews/filters.ttl.jinja', f"view.{view.id}.ttl") as engine:
            engine.template({'id': view.id, 'filters': view.filters})

        with self._templater('ldviews/sources.ttl.jinja', f"view.{view.id}.ttl") as engine:
            engine.template({'id': view.id, 'sources': view.get_sources()})

        with self._templater('ldviews/dimensions.ttl.jinja', f"view.{view.id}.ttl") as engine:
            engine.template({'id': view.id, 'dimensions': view.dimensions})

        projected_dimensions = list(filter(lambda d: d.column is not None, view.dimensions))
        ...
        with self._templater('ldviews/projection.ttl.jinja', f"view.{view.id}.ttl") as engine:
            engine.template({'id': view.id, 'attributes': attributes, 'projected_bnodes': projected_bnodes})
