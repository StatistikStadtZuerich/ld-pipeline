...
class BuildInfo(Copy):
    def __init__(self, output_type: OutputType = OutputType.SHARED, options=None):
        """
        Adds build information to output
        """
        super().__init__(
            source=None, target="info.ttl", output_type=output_type, options=options
        )
    ...
    def run(self, environment: Environment):
        ...
        with tempfile.NamedTemporaryFile(
            mode="wt", prefix="info", suffix=".ttl", delete=False
        ) as tmp:
            tmp.write(content)
